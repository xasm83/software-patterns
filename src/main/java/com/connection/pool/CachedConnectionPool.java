...
//Simple connection pool to model connection pool development
public class CachedConnectionPool implements ConnectionPool {
    ...
    public CachedConnectionPool(String connectionString, int MAX_CONNECTIONS) {
    ...
    public Connection getConnection() throws SQLException, InterruptedException {
        synchronized (pool) {
            while (pool.size() == 0 && activeConnections == MAX_CONNECTIONS) {
                pool.wait();
            }
            if (pool.size() > 0) {
                return pool.pop();
        ...
        return new PooledConnection(DriverManager.getConnection(connectionString));
    }


    //some write methods could be further adjusted or reset() and validate() method could be added to avoid mutation of Connection state
    //close() method is overriden to track usage
    private final class PooledConnection implements Connection {
        final Connection target;

        private PooledConnection(Connection target) {
            this.target = target;
        }
